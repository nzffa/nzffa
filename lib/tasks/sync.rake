...
desc "Make local develoment site look like the staging site"
task :sync_staging => [:environment] do
  host = 'nzffa@nzffa.enspiral.info'
  ssh_host = host
  path = '/home/nzffa/staging/current'
  rsync_command = "rsync -az --progress"
  local_shared_path = "./shared/"
  remote_env = 'remote_staging'

  db_config = YAML.load_file('config/database.yml')
  local_config = db_config[Rails.env]
  password_arg = "-p#{local_config['password']}" if local_config['password']

  echo_and_run "ssh #{ssh_host} \"mysqldump -u #{db_config[remote_env]["username"]} -p#{db_config[remote_env]["password"] } #{db_config[remote_env]["database"]} > ~/dump.sql\""
  echo_and_run "#{rsync_command} #{host}:~/dump.sql ./db/#{remote_env}_data.sql"
  echo_and_run "bundle exec rake db:drop"
  echo_and_run "bundle exec rake db:create"
  echo_and_run "mysql -u #{local_config["username"]} #{local_config["database"]} #{password_arg} < ./db/#{remote_env}_data.sql"
  echo_and_run "#{rsync_command} #{host}:/home/nzffa/staging/shared/public/* #{local_shared_path}"
end
